...
int setfreq(double mhz, bool istx)
{
  ...
  {
    chnn = iio_device_find_channel(get_ad9361_phy(ctx), get_ch_name("altvoltage", 0), true);
  }
  if (chnn == NULL)
    return -2;
...
int setbw(long hz)
{
  return iio_channel_attr_write_longlong(iio_device_find_channel(get_ad9361_phy(ctx),
                                                                 get_ch_name("voltage", 0), false),
                                         "rf_bandwidth", (long long)hz);
}

int setsampel(long hz, bool istx)
{
  return iio_channel_attr_write_longlong(iio_device_find_channel(get_ad9361_phy(ctx),
                                                                 get_ch_name("voltage", 0), istx),
                                         "sampling_frequency", (long long)hz);
}
